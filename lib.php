...
class enrol_openlml_plugin extends enrol_plugin {
	...
	public function sync_user_enrolments($user) {
		...
		if ($this->config->teachers_category_autocreate or $this->config->teachers_category_autoremove) {
			...
			$edited = false;
			if ($this->config->teachers_category_autoremove and (! $this->is_teacher ( $user->username ) or $this->is_ignored_teacher ( $user->username ))) {
				if ($cat = $DB->get_record ( 'course_categories', array (
						'idnumber' => $user->username,
						'parent' => $this->teacher_obj->id 
				), '*', IGNORE_MULTIPLE )) {
					if ($DB->count_records ( 'course_categories', array (
							'idnumber' => $user->username,
							'parent' => $this->teacher_obj->id 
					) ) > 1) {
						if (debugging ())
							trigger_error ( ' There are more than one matching category with idnumber ' . $user->username . ' in ' . $this->teacher_obj->name . ". That is likely to cause problems.", E_USER_WARNING );
					}
			...
			if ($this->config->teachers_category_autocreate and $this->is_teacher ( $user->username ) and ! $this->is_ignored_teacher ( $user->username )) {
				$cat = $DB->get_record ( 'course_categories', array (
						'idnumber' => $user->username,
						'parent' => $this->teacher_obj->id 
				), '*', IGNORE_MULTIPLE );
				if (! $cat) {
				...
				} else if ($DB->count_records ( 'course_categories', array (
						'idnumber' => $user->username,
						'parent' => $this->teacher_obj->id 
				) ) > 1) {
					debugging ( $this->errorlogtag . ' WARNING: there are more than one matching category with idnumber ' . $user->username . ' in ' . $this->teacher_obj->name . ". That is likely to cause problems." );
				}
	...
	public function sync_enrolments() {
		...
		debugging ( $this->errorlogtag . 'sync_enrolments... started ' . date ( "H:i:s" ), DEBUG_DEVELOPER );
		...
		debugging ( $this->errorlogtag . 'sync_enrolments: remove unneeded cohorts... started ' . date ( "H:i:s" ), DEBUG_DEVELOPER );
		$toremove = array ();
		...
		debugging ( $this->errorlogtag . 'sync_enrolments: autoremove teacher course categories ' . ' of removed teachers if requested... started ' . date ( "H:i:s" ), DEBUG_DEVELOPER );
			...
			if ($categories = $teachercontext->get_children ()) {
				foreach ( $categories as $cat ) {
					if (empty ( $cat->idnumber )) {
						debugging ( $this->errorlogtag . sprintf ( 'teacher category %s number %d without teacher userid', $cat->name, $cat->id ) );
						continue;
					}
		...
		debugging ( $this->errorlogtag . 'sync_enrolments: autocreate teacher course categories ' . 'for new teachers if requested... started ' . date ( "H:i:s" ), DEBUG_DEVELOPER );
		if ($this->config->teachers_category_autocreate) {
			foreach ( $this->teacher_array as $teacher ) {
				...
				if (empty ( $cat_obj )) {
					...
					debugging ( $this->errorlogtag . ' WARNING: there are more than one matching category with idnumber ' . $teacher . ' in ' . $this->teacher_obj->name . ". That is likely to cause problems." );
				}
		...
		if ($edited) {
			debugging ( $this->errorlogtag . 'sync_enrolments: resort categories... started ' . date ( "H:i:s" ), DEBUG_DEVELOPER );
			$this->resort_categories ( $this->teacher_obj->id );
		}
	...
	public function update_city(&$user = NULL) {
		...
		if (empty ( $user )) {
			...
			if ($user->city == '') {
				if (! $DB->set_field ( 'user', 'city', $CFG->defaultcity, array (
						'id' => $user->id 
				) )) {
					debugging ( $this->errorlogtag . 'update of city field for user ' . $user->username . " failed." );
				}
			}
		}
	}
	 ...
	 * 
	 * @param eters $courseid
	 *        	: $course->id, $idnumber : $course->idnumber
	...
	private function get_idnumber_cohorts($courseid, $idnumber, $shortname) {
		global $DB;
		$groups = array ();
		if (! isset ( $idnumber ) or empty ( $idnumber ) or ! isset ( $shortname ) or empty ( $shortname ) or ! isset ( $courseid ) or empty ( $courseid )) {
			return $groups;
		}
		if ((strpos ( $idnumber, $shortname . ':' )) === 0) {
			...
			$DB->set_field ( 'course', 'idnumber', $shortname . ':' . implode ( $groups ), array (
					'id' => $courseid 
			) );
		} else {
			$DB->set_field ( 'course', 'idnumber', $shortname . ':' . $idnumber, array (
					'id' => $courseid 
			) );
	...
	private function ldap_get_grouplist($username = "*", $group_pattern = NULL, $all_teachers = false) {
		...
		debugging ( $this->errorlogtag . 'ldap_get_grouplist... started ' . date ( "H:i:s" ), DEBUG_DEVELOPER );
		if (! isset ( $authldap ) or empty ( $authldap )) {
			$authldap = get_auth_plugin ( 'ldap' );
		}
		debugging ( $this->errorlogtag . 'ldap_get_grouplist... ldap_connect ' . date ( "H:i:s" ), DEBUG_DEVELOPER );
		$ldapconnection = $this->ldap_connect_ul ( $authldap );
		debugging ( $this->errorlogtag . 'ldap_get_grouplist... ldap_connected ' . date ( "H:i:s" ), DEBUG_DEVELOPER );
		$fresult = array ();
		if (! $ldapconnection) {
			return FALSE;
		}
		...
		debugging ( $this->errorlogtag . 'ldap_get_grouplist... ldap_close ' . date ( "H:i:s" ), DEBUG_DEVELOPER );
		$authldap->ldap_close ();
		debugging ( $this->errorlogtag . 'ldap_get_grouplist... ldap_closed ' . date ( "H:i:s" ), DEBUG_DEVELOPER );
		if (! $all_teachers) {
	...
	private function ldap_get_group_members($group, $teachers_ok = false) {
		...
		debugging ( $this->errorlogtag . 'ldap_get_group_members(' . $group . ')... started ' . date ( "H:i:s" ), DEBUG_DEVELOPER );
		$ret = array ();
		$members = array ();
		if (! isset ( $authldap ) or empty ( $authldap )) {
			$authldap = get_auth_plugin ( 'ldap' );
		}
		debugging ( $this->errorlogtag . 'ldap_get_groupmembers... ldap_connect ' . date ( "H:i:s" ), DEBUG_DEVELOPER );
		$ldapconnection = $this->ldap_connect_ul ( $authldap );
		debugging ( $this->errorlogtag . 'ldap_get_groupmembers... ldap_connected ' . date ( "H:i:s" ), DEBUG_DEVELOPER );
		...
		debugging ( $this->errorlogtag . 'ldap_get_group_members... connected to ldap ' . date ( "H:i:s" ), DEBUG_DEVELOPER );
		...
		foreach ( $contexts as $context ) {
			...
			debugging ( $this->errorlogtag . sprintf ( 'ldap_get_group_members... ldap_search(%s|%s) %s', $context, $queryg, date ( "H:i:s" ) ), DEBUG_DEVELOPER );
			$resultg = ldap_search ( $ldapconnection, $context, $queryg );
			
			if (! empty ( $resultg ) and ldap_count_entries ( $ldapconnection, $resultg )) {
				debugging ( $this->errorlogtag . 'ldap_get_group_members... ldap_get_entries()' . date ( "H:i:s" ), DEBUG_DEVELOPER );
				$entries = ldap_get_entries ( $ldapconnection, $resultg );
				
				if (isset ( $entries [0] [$this->config->member_attribute] )) {
					debugging ( $this->errorlogtag . sprintf ( 'ldap_get_group_members... entries(%s)(%d) %s', $this->config->member_attribute, count ( $entries [0] [$this->config->member_attribute] ), date ( "H:i:s" ) ), DEBUG_DEVELOPER );
		...
		debugging ( $this->errorlogtag . 'ldap_get_group_members... ldap_close ' . date ( "H:i:s" ), DEBUG_DEVELOPER );
		$authldap->ldap_close ();
		debugging ( $this->errorlogtag . 'ldap_get_groupmembers... ldap_closed ' . date ( "H:i:s" ), DEBUG_DEVELOPER );
		...
		if (isset ( $select )) {
			debugging ( $this->errorlogtag . "ldap_get_group_members... (" . $select . ") " . date ( "H:i:s" ), DEBUG_DEVELOPER );
		} else {
			debugging ( $this->errorlogtag . "ldap_get_group_members... (no selected users) " . date ( "H:i:s" ), DEBUG_DEVELOPER );
		}
		if (isset ( $select )) {
			...
			if (debugging ()) {
				error_log ( $this->errorlogtag . 'ldap_get_group_members... ' . $group . ' is empty. ' . date ( "H:i:s" ) );
			}
		}
		return $ret;
	}
	...
	private function get_cohort_id($groupname, $autocreate = true) {
		...
		debugging ( $this->errorlogtag . 'get_cohort_id(' . $groupname . ')... started ' . date ( "H:i:s" ), DEBUG_DEVELOPER );
		$params = array (
				'idnumber' => $groupname,
		...
		if (! $cohort = $DB->get_record ( 'cohort', $params, '*', IGNORE_MULTIPLE )) {
			...
			if ($DB->count_records ( 'cohort', $params ) > 1) {
				if (debugging ())
					trigger_error ( ' There are more than one matching cohort with idnumber ' . $groupname . '. That is likely to cause problems.', E_USER_WARNING );
			}
			$cohortid = $cohort->id;
		}
		return $cohortid;
	...
	private function get_coursecohortlist($courseid) {
		...
		foreach ( array_keys ( $cohorts ) as $key ) {
			if ($cohorts [$key]->enrol != 'cohort' or ! isset ( $cohorts [$key]->customint1 ) or ! isset ( $cohorts [$key]->customchar1 ) or $cohorts [$key]->customchar1 != $this->enroltype) {
				unset ( $cohorts [$key] );
			} else if ($record = $DB->get_record ( 'cohort', array (
					'id' => $cohorts [$key]->customint1 
			) )) {
				$ret [$record->idnumber] = $record;
			}
		}
	...
	private function get_cohort_members($cohortid) {
		...
		debugging ( $this->errorlogtag . 'get_cohort_members(' . $cohortid . ')... started ' . date ( "H:i:s" ), DEBUG_DEVELOPER );
	...
	private function is_teacher($userid) {
		debugging ( $this->errorlogtag . 'is_teacher(' . $userid . ')... started ' . date ( "H:i:s" ), DEBUG_DEVELOPER );
		if (empty ( $userid )) {
	...
	private function init_teacher_array() {
		global $CFG;
		debugging ( $this->errorlogtag . 'init_teacher_array... started ' . date ( "H:i:s" ), DEBUG_DEVELOPER );
		$this->teacher_array = $this->ldap_get_group_members ( $this->config->teachers_group_name, true );
		debugging ( $this->errorlogtag . 'init_teacher_array... ended ' . date ( "H:i:s" ), DEBUG_DEVELOPER );
		if (empty ( $this->teacher_array )) {
			return false;
		}
		return true;
	...
	private function get_teacher_category() {
		...
		$cat_obj = $DB->get_record ( 'course_categories', array (
				'idnumber' => $this->idnumber_teachers_cat,
				'parent' => 0 
		), '*', IGNORE_MULTIPLE );
		if (! $cat_obj) { // Category doesn't exist.
			$cat_obj = $this->create_category ( $this->config->teachers_course_context, $this->idnumber_teachers_cat, get_string ( 'teacher_context_desc', 'enrol_openlml' ) );
	...
	private function get_teacher_attic_category() {
		global $CFG, $DB;
		$this->attic_obj = $DB->get_record ( 'course_categories', array (
				'idnumber' => $this->idnumber_attic_cat,
				'parent' => 0 
		), '*', IGNORE_MULTIPLE );
		if (! $this->attic_obj) { // Category for removed teachers doesn't exist.
			$this->attic_obj = $this->create_category ( $this->config->teachers_removed, $this->idnumber_attic_cat, get_string ( 'attic_description', 'enrol_openlml' ), 0, 99999, 0 );
		...
		if ($categories = $teacher_cat->get_children ()) {
			...
			foreach ( $categories as $cat ) {
				if ($cat->sortorder != $count) {
					$DB->set_field ( 'course_categories', 'sortorder', $count, array (
							'id' => $cat->id 
					) );
	...
	private function teacher_add_category(&$user) {
		...
		if (empty ( $user->username ) || empty ( $user->firstname ) || empty ( $user->lastname )) {
			debugging ( $this->errorlogtag . sprintf ( 'teacher_add_category: data missing(userid:%s|firstname:%s|lastname:%s)', $user->username, $user->firstname, $user->lastname ) );
			return false;
		}
		if (! isset ( $this->attic_obj )) {
			$this->attic_obj = $this->get_teacher_attic_category ();
		...
		if ($cat_obj) {
			...
			$description = get_string ( 'course_description', 'enrol_openlml' ) . ' ' . $user->firstname . ' ' . $user->lastname . '(' . $user->username . ').';
			$cat_obj = $this->create_category ( $user->lastname . "," . $user->firstname, $user->username, $description, $this->teacher_obj->id );
			if (! $cat_obj) {
	...
	private function teacher_has_role($user, $cat) {
		...
		return $DB->record_exists ( 'role_assignments', array (
				'roleid' => $this->config->teachers_course_role,
				'contextid' => $teacherscontext->id,
				'userid' => $user->id,
				'component' => 'enrol_openlml' 
		) );
	}
	 ...
	 * 
	 * @param $user teacher
	 *        	for whom the coursecreator role will be added
	 * @param $cat category
	 *        	for whom to add the teacher as role teachers_course_role
	 * @return false|true
	 * @uses $CFG;
	 */
	private function teacher_assign_role($user, $cat) {
		...
		if (! role_assign ( $this->config->teachers_course_role, $user->id, $teacherscontext, 'enrol_openlml' )) {
			debugging ( $this->errorlogtag . 'could not assign role (' . $this->config->teachers_course_role . ') to user (' . $user->username . ') in context (' . $teacherscontext->id . ').' );
			return false;
		}
	 ...
	 * 
	 * @param $user teacher
	 *        	for whom the coursecreator role will be removed
	 * @param $cat category
	 *        	for whom to remove the teacher as role teachers_course_role
	 * @return false|true
	 * @uses $CFG;
	 */
	...
	public function create_category($name, $idnumber, $description, $parent = 0, $sortorder = 0, $visible = 1) {
		...
		debugging ( $this->errorlogtag . sprintf ( "create_category... %s (%s),\n                sortorder(%s) %s", $name, $description, $sortorder, date ( "H:i:s" ) ), DEBUG_DEVELOPER );
		$data = new stdClass ();
		$data->name = $name;
		...
		if ($sortorder != 0) {
			debugging ( $this->errorlogtag . 'Changing course sortorder(' . $sortorder . ') ' . date ( "H:i:s" ), DEBUG_DEVELOPER );
			$DB->set_field ( 'course_categories', 'sortorder', $sortorder, array (
					'id' => $cat->id 
			) );
			context_coursecat::instance ( $cat->id )->mark_dirty ();
	...
	private function ldap_connect_ul($authldap) {
		...
		if ($ldapconnection = ldap_connect_moodle ( $authldap->config->host_url, $authldap->config->ldap_version, $authldap->config->user_type, $binddn, $bindpw, $optderef, $debuginfo, $starttls )) {
